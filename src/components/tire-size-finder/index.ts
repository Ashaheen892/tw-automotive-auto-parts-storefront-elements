...
/* ---- inlined: components/tire-size-finder/styles.ts ---- */
...
/* ---- inlined: components/tire-size-finder/types.ts ---- */
type TireTypeId = string;

interface TireTypeOption {
  id: string;
  name: string;
  desc: string;
}

interface TireSizeRow {
  id: string;
  width: string;
  aspect: string;
  rim: string;
...
/* ---- inlined: components/tire-size-finder/utils.ts ---- */
...
function parseTireTypes(raw: unknown): TireTypeOption[] {
...
function parseSizeList(raw: unknown, fallback: string[]): string[] {
...
function resolveWidths(config: Record<string, unknown>): string[] {
  return parseSizeList(config.tsf_widths, DEFAULT_WIDTHS);
}

function resolveAspects(config: Record<string, unknown>): string[] {
  return parseSizeList(config.tsf_aspects, DEFAULT_ASPECTS);
}

function resolveRims(config: Record<string, unknown>): string[] {
  return parseSizeList(config.tsf_rims, DEFAULT_RIMS);
}

function parseTireSizes(raw: unknown): TireSizeRow[] {
...
function formatTireCode(width: string, aspect: string, rim: string): string {
...
/* ---- inlined: components/tire-size-finder/index.ts ---- */
