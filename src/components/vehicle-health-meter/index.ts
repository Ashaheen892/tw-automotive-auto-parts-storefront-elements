...
/* ---- inlined: components/vehicle-health-meter/styles.ts ---- */
...
/* ---- inlined: components/vehicle-health-meter/types.ts ---- */
type MeterStatus = 'excellent' | 'good' | 'check' | 'service';

interface HealthMeter {
  id: string;
  name: string;
  value: number;
  status: MeterStatus;
...
type MeterDisplay = 'circles' | 'bars';

/* ---- inlined: components/vehicle-health-meter/utils.ts ---- */
function resolveDisplay(value: unknown): MeterDisplay {
  const raw = getRadioValue(value, 'circles').toLowerCase();
  return raw === 'bars' ? 'bars' : 'circles';
}

function resolveStatus(raw: unknown, value: number): MeterStatus {
...
function statusLabel(status: MeterStatus): string {
...
function parseMeters(raw: unknown): HealthMeter[] {
...
function circleDashOffset(value: number, radius = 42): number {
...
/* ---- inlined: components/vehicle-health-meter/index.ts ---- */
