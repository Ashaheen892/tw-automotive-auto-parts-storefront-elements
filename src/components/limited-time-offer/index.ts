...
/* ---- inlined: components/limited-time-offer/styles.ts ---- */
...
/* ---- inlined: components/limited-time-offer/types.ts ---- */
type CountdownParts = {
  days: number;
  hours: number;
  minutes: number;
...
/* ---- inlined: components/limited-time-offer/utils.ts ---- */
const DEFAULT_IMAGE =
  'https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?auto=format&fit=crop&w=1400&q=80';

function parseEndsAt(raw: unknown): Date | null {
...
function getCountdown(target: Date, now = new Date()): CountdownParts {
...
function resolveOfferImage(config: Record<string, unknown>): string {
  return (
...
function resolveCtaLink(config: Record<string, unknown>): string {
  return extractLink(config.lto_cta_link) || extractLink(config.lto_link);
}

function showTimerUnit(config: Record<string, unknown>, key: string, fallback = true): boolean {
  return isTruthy(config[key], fallback);
}

function imageOnStart(config: Record<string, unknown>): boolean {
  const raw = String(config.lto_image_side || config.lto_media_side || 'end').toLowerCase();
  return raw === 'start' || raw === 'left' || raw === 'begin';
}

/* ---- inlined: components/limited-time-offer/index.ts ---- */
