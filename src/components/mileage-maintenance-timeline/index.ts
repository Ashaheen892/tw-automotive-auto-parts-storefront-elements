...
/* ---- inlined: components/mileage-maintenance-timeline/styles.ts ---- */
...
/* ---- inlined: components/mileage-maintenance-timeline/types.ts ---- */
...
type MilestoneStatus = 'done' | 'due' | 'upcoming' | 'neutral';

/* ---- inlined: components/mileage-maintenance-timeline/utils.ts ---- */
/** Split services from newline or comma-separated text. */
function splitServices(raw: LocaleValue): string[] {
...
function parseMilestones(raw: unknown): MaintenanceMilestone[] {
...
function formatKm(km: number, unitLabel: string): string {
  const formatted = Number.isFinite(km) ? km.toLocaleString() : '0';
  return unitLabel ? `${formatted} ${unitLabel}` : formatted;
}

function showOdometer(config: Record<string, unknown>): boolean {
...
function progressPercent(milestones: MaintenanceMilestone[], currentKm: number | null): number {
...
function pinLabel(milestone: MaintenanceMilestone, index: number): string {
  const raw = String(milestone.icon || '').trim();
  if (raw && raw.length <= 3 && !/\p{Extended_Pictographic}/u.test(raw)) return raw;
  return String(index + 1).padStart(2, '0');
}

/* ---- inlined: components/mileage-maintenance-timeline/index.ts ---- */
