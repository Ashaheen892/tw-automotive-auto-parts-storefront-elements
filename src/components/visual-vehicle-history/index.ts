...
/* ---- inlined: components/visual-vehicle-history/styles.ts ---- */
...
/* ---- inlined: components/visual-vehicle-history/types.ts ---- */
...
type HistoryLayout = 'vertical' | 'horizontal';

/* ---- inlined: components/visual-vehicle-history/utils.ts ---- */
function resolveLayout(value: unknown): HistoryLayout {
  const raw = getRadioValue(value, 'vertical').toLowerCase();
  return raw === 'horizontal' ? 'horizontal' : 'vertical';
}

function showStats(config: Record<string, unknown>): boolean {
  return isTruthy(config.vvh_show_stats, true);
}

function parseEvents(raw: unknown): HistoryEvent[] {
...
function latestEvent(events: HistoryEvent[]): HistoryEvent | null {
  return events[0] ?? null;
}

function nextServiceHint(events: HistoryEvent[]): string {
...
function uniqueCategories(events: HistoryEvent[]): string[] {
...
/* ---- inlined: components/visual-vehicle-history/index.ts ---- */
